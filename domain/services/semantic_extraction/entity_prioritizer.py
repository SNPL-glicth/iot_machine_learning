...
class PrioritizationResult:
    ...
    def to_enrichment_result(
        ...
        entities = getattr(original, "entities", [])
        entity_count = getattr(original, "entity_count", len(entities))
        equipment_metric_pairs = getattr(original, "equipment_metric_pairs", [])
        domain_detected = getattr(original, "domain_detected", "general")
        enrichment_confidence = getattr(
            original, "enrichment_confidence", getattr(original, "confidence_aggregate", 0.0)
        )
        return SemanticEnrichmentResult(
            entities=entities,
            critical_entities=critical,
            entity_count=entity_count,
            equipment_metric_pairs=equipment_metric_pairs,
