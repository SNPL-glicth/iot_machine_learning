...
def _parse_cognitive_sensor_ids() -> Optional[Set[int]]:
    """FIX P2-3: Parse ML_COGNITIVE_SENSOR_IDS env var."""
    raw = os.getenv("ML_COGNITIVE_SENSOR_IDS", "").strip()
    if not raw:
        return set()
    if raw == "*":
        return None  # All sensors
    try:
...
class PredictionDomainService:
    ...
    def __init__(
        ...
        self._audit = audit
        self._cognitive_sensor_ids = _parse_cognitive_sensor_ids()
    ...
    def predict(self, window: SensorWindow) -> Prediction:
        ...
        # Seleccionar motor (FIX P2-3: cognitive opt-in per sensor)
        engine = self._select_engine(window.size, window.sensor_id)
    ...
    def _is_cognitive_allowed(self, sensor_id) -> bool:
        """FIX P2-3: Check if cognitive engines are allowed for this sensor."""
        if self._cognitive_sensor_ids is None:
            return True  # '*' → all
        try:
            return int(sensor_id) in self._cognitive_sensor_ids
        except (ValueError, TypeError):
            return False

    def _select_engine(self, n_points: int, sensor_id=None) -> PredictionPort:
        """Selecciona el primer motor que pueda manejar n_points.

        FIX P2-3: Skips cognitive engines if sensor not in ML_COGNITIVE_SENSOR_IDS.
        """
        cognitive_allowed = self._is_cognitive_allowed(sensor_id) if sensor_id else True
        for engine in self._engines:
            if not cognitive_allowed and "cognitive" in engine.name.lower():
                continue
