"""ML State Store IoT Adapter — implements TelemetryStoragePort.

Provides in-memory fallback and Redis-backed state snapshot storage for ML engines.
Implements the pure TelemetryStoragePort contract defined in ML Core.
"""
...
class InMemoryMLStateStore(TelemetryStoragePort):
    """Process-local state store. Useful for testing and graceful offline fallback."""
    ...
    def save_snapshot(self, engine_id: str, payload: Dict[str, Any]) -> bool:
        return self.save(engine_id, payload)
    ...
    def delete_snapshot(self, engine_id: str) -> bool:
        return self.delete(engine_id)


class RedisMLStateStore(TelemetryStoragePort):
    """Redis-backed state store with configurable TTL. One key per engine snapshot."""
    ...
    def __init__(
        ...
        import redis  # Lazy import keeps this module importable without redis.
...
def create_state_store(config: Optional[Dict[str, Any]]) -> Optional[TelemetryStoragePort]:
