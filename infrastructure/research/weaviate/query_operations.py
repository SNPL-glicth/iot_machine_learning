...
def graphql_near_vector(
    ...
    *,
    ...
    """Execute a nearVector GraphQL query using an explicit numeric vector.

    Uses Weaviate's ``nearVector`` operator against a pre-stored numeric
    vector field (e.g. ``situationVector``).  Does **not** rely on
    text2vec-transformers.

    Args:
        graphql_url: Full URL to /v1/graphql endpoint
        class_name: Weaviate class name
        vector: Numeric vector (e.g. 18-dim situation vector)
        return_fields: List of field names to return
        where_filter: Optional where filter dict
        limit: Max results to return
        certainty: Minimum certainty threshold (0.0-1.0)
    ...
    near_vector = f'nearVector: {{ vector: {vector_json}, certainty: {certainty} }}'
    ...
    query = (
        "{ Get { "
        f"{class_name}({near_vector}{where_clause}, limit: {limit}) "
        f"{{ {fields_str} {additional} }} "
        "} }"
    )
