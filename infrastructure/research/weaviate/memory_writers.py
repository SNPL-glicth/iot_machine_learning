...
def _build_natural_explanation_text(
    prediction: Prediction,
    explanation_text: str = "",
) -> str:
    """Build a human-readable explanation text for Weaviate vectorization.

    Replaces raw dict serialization with a natural sentence that
    Weaviate's text2vec-transformers can actually embed meaningfully.
    """
    if explanation_text and not explanation_text.strip().startswith("{"):
        return explanation_text

    exp = prediction.metadata.get("explanation")
    regime = "unknown"
    summary = ""

    if isinstance(exp, dict):
        regime = str(exp.get("signal", {}).get("regime", "unknown"))
        try:
            summary = json.dumps(exp, ensure_ascii=False, separators=(",", ":"))
        except Exception:
            summary = str(exp)
        if len(summary) > 200:
            summary = summary[:197] + "..."
    ...
    return (
        f"Regime {regime} | Engine {prediction.engine_name} | "
        f"Trend {prediction.trend} | Confidence {prediction.confidence_score:.2f} | "
        f"{summary}"
    )
...
def remember_explanation(
    ...
    *,
    ...
    """
    try:
        text = _build_natural_explanation_text(prediction, explanation_text)
        sv = prediction.metadata.get("situation_vector")
        props = {
            ...
            "situationVector": sv if isinstance(sv, list) else [],
        }
