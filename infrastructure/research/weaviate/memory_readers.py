...
from .query_operations import graphql_near_text, graphql_near_vector
...
def recall_similar_by_vector(
    ...
    *,
    ...
    """Recall similar prediction explanations using numeric situation vector.

    Uses ``nearVector`` against the ``situationVector`` field in Weaviate.
    Falls back to empty results if the vector is empty (caller should then
    use ``recall_similar_explanations`` with nearText).

    Args:
        graphql_url: Full URL to /v1/graphql endpoint
        situation_vector: 18-dim numeric vector from SituationVectorBuilder
        series_id: Filter by series ID (optional)
        engine_name: Filter by engine name (optional)
        limit: Max results to return
        min_certainty: Minimum certainty threshold
    ...
    Returns:
        List of MemorySearchResult objects ordered by similarity descending.
    """
    if not situation_vector or len(situation_vector) == 0:
        return []
