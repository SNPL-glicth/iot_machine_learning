...
.. deprecated:: 2.0
    ...
    New code should use ``BaselineMovingAverageEngine(...).as_port()``
    or ``PredictionEnginePortBridge(engine)`` instead.
...
class BaselinePredictionAdapter(PredictionPort):
    ...
    def predict(self, window: SensorWindow) -> Prediction:
        ...
        """
        from iot_machine_learning.infrastructure.ml.engines.baseline.engine import BaselineConfig, predict_moving_average
