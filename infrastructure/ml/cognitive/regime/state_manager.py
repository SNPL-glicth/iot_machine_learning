...
Maintains historical regime states per series with smooth transitions.
"""
...
max_history_size: Maximum number of states to keep per series
    ...
    def smooth_transition(
        ...
        # Get current state
        current_state = self._get_current_state(series_id)
    ...
    def get_previous_regime(self, series_id: int) -> Optional[str]:
        """
        Get previous regime for a series.
        ...
        """
        history = self._state_history.get(series_id)
        if history and len(history) >= 2:
            return history[-2].regime
        return None
    
    def get_transition_duration(self, series_id: int) -> Optional[float]:
        """
    ...
    def get_regime_duration(self, series_id: int) -> Optional[float]:
        """
    ...
    def get_current_regime(self, series_id: int) -> Optional[str]:
        """
        Get current regime for a series.
    ...
    def _get_current_state(self, series_id: int) -> Optional[RegimeState]:
        """Get current state for a series."""
        history = self._state_history.get(series_id)
        if history and len(history) > 0:
            return history[-1]
        return None
    
    def _add_state(self, series_id: int, regime: str, timestamp: float) -> None:
        """Add a new state to history."""
        if series_id not in self._state_history:
            self._state_history[series_id] = deque(maxlen=self._max_history_size)
        
        self._state_history[series_id].append(RegimeState(regime=regime, timestamp=timestamp))
    
    def _update_state(self, series_id: int, timestamp: float) -> None:
    ...
    def cleanup_series(self, series_id: int) -> None:
        """
        Remove all states for a series (for cleanup).
        
        Args:
            series_id: Series identifier
        """
        self._state_history.pop(series_id, None)
