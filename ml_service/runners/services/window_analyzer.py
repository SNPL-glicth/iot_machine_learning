...
This module implements sliding window analysis and pattern classification for IoT sensor data.
Part of the ZENIN ML cognitive pipeline for real-time behavioral analysis.

Extracted from ml_stream_runner.py for modularity.
Responsibility: Analyze sliding windows and classify behavior patterns.

Security: No PII processed. All sensor values are validated and sanitized before analysis.
Window statistics are computed using safe numerical methods to prevent overflow.
"""
...
try:
    ...
    from iot_machine_learning.ml_service.runners.models.online_analysis import OnlineAnalysis
    from iot_machine_learning.ml_service.runners.models.sensor_state import SensorState
...
class WindowAnalyzer:
    ...
    def __init__(
        ...
        stable_trend_threshold: float = 0.05,
        stable_var_threshold: float = 0.01,
        stable_z_score_threshold: float = 1.0,
        oscillation_var_threshold: float = 0.05,
        drifting_trend_threshold: float = 0.05,
        drifting_var_threshold: float = 0.05,
    ) -> None:
        self._cfg = cfg
        self._stable_trend_threshold = stable_trend_threshold
        self._stable_var_threshold = stable_var_threshold
        self._stable_z_score_threshold = stable_z_score_threshold
        self._oscillation_var_threshold = oscillation_var_threshold
        self._drifting_trend_threshold = drifting_trend_threshold
        self._drifting_var_threshold = drifting_var_threshold
    ...
    def _classify_pattern(
        ...
        *,
        ...
        if (
            abs(trend_long) < self._stable_trend_threshold
            and var_long < self._stable_var_threshold
            and abs(z_score_last) < self._stable_z_score_threshold
        ):
            return "STABLE"

        # Oscilación: alta variabilidad y cambios de signo en la pendiente
        if var_long >= self._oscillation_var_threshold:
        ...
        if abs(trend_long) >= self._drifting_trend_threshold and var_long < self._drifting_var_threshold:
            return "DRIFTING"
