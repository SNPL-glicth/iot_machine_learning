"""
SKAB (Skoltech Anomaly Benchmark) Benchmark
Compara VotingAnomalyDetector vs baselines para detección de anomalías en datos industriales.
...
Output:
    results/skab_results.json
    results/skab_report.md
    results/skab_plot.png
"""
...
logger = logging.getLogger("skab_benchmark")

# ─── Paths ───────────────────────────────────────────────────────────────────
RESULTS_DIR = Path("results/Datasets")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
DATA_DIR = Path("/tmp/SKAB")
DATA_DIR.mkdir(parents=True, exist_ok=True)

# SKAB Data URL
SKAB_URL = "https://github.com/waico/SKAB/raw/master/"

# ─── Config benchmark ────────────────────────────────────────────────────────
WINDOW_SIZE = 50  # Ventana para detección
SERIES_ID = "SKAB-anomaly-detection"

# SKAB files to process
SKAB_FILES = [
    "data/valve1/1.csv",
    "data/valve1/2.csv", 
    "data/valve2/1.csv",
...
def download_skab_data():
    """Descarga los datos de SKAB si no existen."""
    all_exist = True
    for file_path in SKAB_FILES:
        local_path = DATA_DIR / file_path.replace("/", "_")
        if not local_path.exists():
            all_exist = False
            break
    
    if all_exist:
        logger.info("Datos de SKAB ya existen, omitiendo descarga")
        return [DATA_DIR / f.replace("/", "_") for f in SKAB_FILES]
    
    logger.info("Descargando datos de SKAB...")
    
    downloaded_files = []
    for file_path in SKAB_FILES:
        filename = file_path.replace("/", "_")
        local_path = DATA_DIR / filename
        url = SKAB_URL + file_path
...
def load_skab_data() -> Tuple[List[float], List[float], List[int]]:
    """
    Carga dataset SKAB (Skoltech Anomaly Benchmark).
    
    Returns: values, timestamps, labels (0/1 por punto)
    """
    downloaded_files = download_skab_data()
    
    if not downloaded_files:
        raise ValueError("No se pudieron descargar archivos SKAB")
    
    logger.info(f"Cargando {len(downloaded_files)} archivos SKAB...")
    ...
    for file_path in downloaded_files:
        try:
            ...
            # Buscar columna de anomalía
            anomaly_col = None
            for col in df.columns:
                if 'anomaly' in col.lower() or 'changepoint' in col.lower():
                    anomaly_col = col
                    break
            
            if anomaly_col is None:
                logger.warning(f"No se encontró columna de anomalía en {file_path.name}, usando Pressure")
                signal_col = 'Pressure' if 'Pressure' in df.columns else df.select_dtypes(include=[np.number]).columns[0]
                values = df[signal_col].values
                labels = np.zeros(len(values))
            else:
                # SKAB: Volume Flow RateRMS best signal (F1=0.533 vs Pressure F1=0.436)
                if 'Volume Flow RateRMS' in df.columns:
                    signal_col = 'Volume Flow RateRMS'
                elif 'Pressure' in df.columns:
                    signal_col = 'Pressure'
                else:
                    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                    numeric_cols = [c for c in numeric_cols if c != anomaly_col]
                    signal_col = numeric_cols[0] if numeric_cols else None
                    if signal_col is None:
                        continue
                values = df[signal_col].values
                labels = df[anomaly_col].values
            
            labels = np.array(labels, dtype=int)
            start_ts = len(all_values)
            all_values.extend(values.astype(float))
            all_labels.extend(labels)
            all_timestamps.extend(np.arange(start_ts, start_ts + len(values)))
            
            logger.info(f"Cargado {file_path.name}: {len(values)} puntos, {int(sum(labels))} anomalías")
    ...
    if not all_values:
        raise ValueError("No se pudieron cargar datos de SKAB")
...
def generate_report(
    ...
    json_path = RESULTS_DIR / "skab_results.json"
    payload = {
    ...
    # 2. Markdown report
    md_path = RESULTS_DIR / "skab_report.md"
    zenin = next(r for r in results if r.name == "ZENIN VotingEnsemble")
    with open(md_path, "w") as f:
        f.write("# SKAB Benchmark — Skoltech Anomaly Benchmark\n\n")
        f.write("**Dataset:** `SKAB (Skoltech Anomaly Benchmark)`\n")
        ...
        f.write("\n## Dataset Description\n\n")
        f.write("The SKAB (Skoltech Anomaly Benchmark) dataset contains real-world ")
        f.write("industrial sensor data with labeled anomalies from various equipment ")
        f.write("including valves and other industrial components. It includes different ")
        f.write("types of anomalies such as leaks, blockages, and sensor failures.\n")

    logger.info(f"Markdown guardado: {md_path}")

    # 3. PNG plot
    plot_path = RESULTS_DIR / "skab_plot.png"
    fig, axes = plt.subplots(3, 1, figsize=(15, 10))
    
    # Plot 1: Raw data with anomalies
    plot_points = min(1000, len(values))
    axes[0].plot(values[:plot_points], label='Sensor Signal', alpha=0.7)
    ...
    axes[0].set_title('SKAB: Sensor Data with Anomalies (First 1000 points)')
...
def main():
    logger.info("=" * 70)
    logger.info("SKAB BENCHMARK — Skoltech Anomaly Benchmark")
    logger.info("=" * 70)
    
    # Cargar datos
    values, timestamps, labels = load_skab_data()
    ...
    _zm, _zs = float(np.mean(values)), float(np.std(values))
    zscore_scores = [float(abs((v - _zm) / _zs)) for v in values]
    ...
    _iq1, _iq3 = np.percentile(values, 25), np.percentile(values, 75)
    _iqr = _iq3 - _iq1 if _iq3 - _iq1 > 0 else 1e-10
    _im = np.median(values)
    iqr_scores = [float(abs(v - _im) / _iqr) for v in values]
