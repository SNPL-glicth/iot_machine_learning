...
def main():
    ...
    for sheet_name, df in sheets.items():
        ...
        for col in param_cols:
            ...
            # ── Performance tracking (observer mode) ──
            # Alimenta el ExpertPerformanceTracker con el error
            # normalizado de cada predicción para que _adjust_by_performance()
            # use data real en vez del default 0.5.
            if pred_val is not None and metrics.get("mae") is not None:
                try:
                    norm_error = min(metrics["mae"] / (abs(ctx.mean) + 1e-10), 1.0)
                    engine._gating._performance_tracker.record(te, norm_error)
                except Exception:
                    pass
