...
def test_models(df_pivot, sheet_name):
    ...
    # Filtrar columnas constantes (std≈0) que desestabilizan StandardScaler
    non_constant_cols = [c for c in numeric_cols if df_clean[c].std() > 1e-10]
    if len(non_constant_cols) < 2:
        print("Menos de 2 columnas no constantes, se requiere para modelos multivariados")
        return
    dropped = set(numeric_cols) - set(non_constant_cols)
    if dropped:
        print(f"  Columnas constantes excluidas: {sorted(dropped)}")
    
    # Escalar solo con columnas no constantes
    scaler = StandardScaler()
    df_scaled = pd.DataFrame(scaler.fit_transform(df_clean[non_constant_cols]), 
