...
Migrated from deprecated TaylorPredictionAdapter to use
PredictionEnginePortBridge via engine.as_port() pattern.

KalmanFilterAdapter moved to filters/ package.
"""
...
from iot_machine_learning.infrastructure.ml.filters import KalmanFilterAdapter
...
class TestTaylorEngineAsPort:
    """Tests for TaylorPredictionEngine.as_port() bridge."""

    def test_as_port_implements_prediction_port(self) -> None:
        """engine.as_port() debe ser instancia de PredictionPort."""
        engine = TaylorPredictionEngine(order=2, horizon=1)
        adapter = engine.as_port()
        assert isinstance(adapter, PredictionPort)

    def test_as_port_name_is_taylor(self) -> None:
        """Nombre del port debe ser 'taylor'."""
        engine = TaylorPredictionEngine()
        adapter = engine.as_port()
        assert adapter.name == "taylor"
    ...
    def test_different_orders(self) -> None:
        ...
        for order in (1, 2, 3):
            engine = TaylorPredictionEngine(order=order, horizon=1)
            adapter = engine.as_port()
            window = _make_window(1, [20.0 + i * 0.1 for i in range(20)])
            result = adapter.predict(window)
            assert isinstance(result, Prediction)
