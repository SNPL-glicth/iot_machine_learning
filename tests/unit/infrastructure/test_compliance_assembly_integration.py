"""AssemblyPhase ↔ ComplianceExporter integration (IMP-5 §4, PIPE-2).

Covers:
* Export happens when compliance_exporter is injected.
* Export is skipped when exporter is None.
* Export failure does not corrupt the returned PredictionResult.
* Two predictions with the SAME exporter instance append to the same sink.
"""
...
class TestComplianceAssemblyIntegration:
    def test_assembly_phase_exports_when_exporter_injected(self, tmp_path: Path) -> None:
        sink = tmp_path / "audit.ndjson"
        exporter = ComplianceExporter(sink_path=sink)
        result = AssemblyPhase(compliance_exporter=exporter).execute(_build_ctx())
        assert sink.exists()
    ...
    def test_assembly_phase_skips_export_when_exporter_none(self, tmp_path: Path) -> None:
        # No exporter injected — no sink file should appear.
        sink = tmp_path / "audit.ndjson"
        result = AssemblyPhase(compliance_exporter=None).execute(_build_ctx())
    ...
    def test_exporter_reused_across_calls(self, tmp_path: Path) -> None:
        """Two predictions with the same exporter append to the SAME sink file."""
        sink = tmp_path / "audit.ndjson"
        exporter = ComplianceExporter(sink_path=sink)
        AssemblyPhase(compliance_exporter=exporter).execute(_build_ctx())
        AssemblyPhase(compliance_exporter=exporter).execute(_build_ctx())
